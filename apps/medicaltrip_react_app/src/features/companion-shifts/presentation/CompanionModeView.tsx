...
export const CompanionModeView: React.FC = () => {
  ...
  // Math: $15.500/h + $15.500 prep allowance + meal subsidy
  const hourlySubtotalCOP = Math.round(hoursLogged * 15500);
  const prepAllowanceCOP = 15500;
  const mealSubsidyAmountCOP = useMemo(() => {
    return CompanionShift.resolveMealSubsidyAmount(selectedMealTier).toAmountNumber();
  }, [selectedMealTier]);
  ...
  return (
    ...
    >
      ...
      <section className="bg-zinc-950 border-b border-zinc-800/80 px-4 py-2.5 flex flex-wrap items-center justify-between gap-2 text-xs">
        <div className="flex items-center gap-2">
          <span className="text-base" role="img" aria-label="Bandera Curazao">🇨🇼</span>
          <span className="font-bold text-white tracking-wide">{patientFirstName} {patientLastName}</span>
      ...
      <main className="max-w-2xl mx-auto w-full p-4 space-y-5">
        {/* CARD 1: SHIFT TIMER & HOURLY BILLING */}
        <section className="bg-zinc-950 border border-zinc-800 rounded-3xl p-5 shadow-sm space-y-4">
          ...
          <div className="flex items-center justify-between gap-3 bg-zinc-900/80 p-3.5 rounded-2xl border border-zinc-800">
            ...
            <div className="text-center">
              ...
              <span className="text-[11px] text-amber-400 font-medium">Paso de 30 min (0.5h)</span>
            </div>
        ...
        {/* CARD 2: 1-CLICK LUNCH SUBSIDY TIERS */}
            ...
            {MEAL_TIER_DETAILS.map((opt) => {
              ...
              return (
                <button
                  ...
                  data-testid={testId}
                  onClick={() => setSelectedMealTier(opt.tier)}
        ...
        {/* CARD 3: 1-TAP PETTY CASH & OCR SCANNER */}
        <section className="bg-zinc-950 border border-zinc-800 rounded-3xl p-5 shadow-sm space-y-3.5">
          <div className="flex items-center justify-between border-b border-zinc-800 pb-2.5">
            <div className="flex items-center gap-2">
      ...
      {isOcrOpen && (
        <ReceiptOcrModal
          ...
          onApproved={(expense) => {
            setIsOcrOpen(false);
            setQuickNotification(
              `✅ Factura de ${expense.vendorName || 'farmacia'} aprobada e ingresada por ${expense.amount.format()}.`
            );
